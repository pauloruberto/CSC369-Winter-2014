/*
To improve efficiency I did:
 - seperated the if statements in the mymalloc()'s while loop so it will only
   check the memchunk's size if it is first free. This removes unneccesary arithmetic

 - augmented the freelist to point to the next free blocks that mymalloc will iterate through.

 - got rid of redundant error checks, will assume mymalloc() and myfree() behave nicely

I focused on increasing speed at the sacrifice of space because many computers these
days can easily expand their max space but have a bottleneck in terms of computation
speed. It may seem like I may have to do a lot of iterations every time I call splitAndUse()
but as the heap grows and becomes more and more split the main while loop in mymalloc() will
become more efficient since it will operate like a skip list.
...
typedef struct __node {
  ...
  struct __node *next_free; //next free memchunk
...
void *splitAndUse(node *memchunk, unsigned int size) {
  ...
  node *old_next_free = memchunk->next_free;

  memchunk->isFree = 0;
  memchunk->size = nodeSize + size; //header and data
  memchunk->next = new_node;

  //update all the previous node's next_free pointers that are in use 
  memchunk->next_free = new_node;
  node *copy_memchunk;
  for(copy_memchunk = memchunk; copy_memchunk != NULL; copy_memchunk = copy_memchunk->prev) {
    if (copy_memchunk->isFree == 1)
      break;
    
    copy_memchunk->next_free = new_node;
  }
  ...
  new_node->next_free = old_next_free;
...
void coalesce(node *memchunk) {
  ...
  memchunk->next_free = memchunk->next->next_free;
...
void *mymalloc(unsigned int size) {
  ...
  for(memchunk = freelist_head; memchunk != NULL; memchunk = memchunk->next_free) {
    //if there is a free memchunk that is big enough to hold size + node
    if (memchunk->isFree == 1)
      if (memchunk->size >= size + 2*sizeof(node))
        break;

    if (memchunk->next == NULL) {
      memchunk->next = (node *) getNewPage();
      memchunk->next->next = NULL;
      memchunk->next->next_free = NULL;
      memchunk->next->prev = memchunk;
